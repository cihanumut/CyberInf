...
export default function PostDetail() {
  ...
  const handleCommentLike = async (commentId, blogId) => {
    if (!isAuthenticated) return toast.error("Beğenmek için giriş yap.");
    try {
      const { data } = await commentService.toggleCommentLike(
        blogId,
        commentId,
      );
      setComments((prev) =>
  ...
  const isAuthor =
    user?._id === blog.author?._id || user?.id === blog.author?._id;

  return (
    <div
      className="container"
      style={{ padding: "2rem 1.5rem 4rem", maxWidth: 860 }}
    >
      ...
      <div>
        <h2 style={{ fontFamily: "var(--font-mono)", marginBottom: "1.5rem" }}>
          <span style={{ color: "var(--accent-green)" }}>//</span> Yorumlar (
          {comments.length})
        </h2>
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        gap: "0.25rem",
                        color: c.likes?.includes(user?.id || user?._id)
                          ? "var(--accent-red, #f85149)"
                          : "var(--text-muted)",
                        fontSize: "0.75rem",
                        fontFamily: "var(--font-mono)",
                    ...
                    >
                      {c.likes?.includes(user?.id || user?._id) ? "❤️" : "🤍"}{" "}
